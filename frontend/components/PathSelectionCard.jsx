...
export default function PathSelectionCard({ icon: Icon, title, description, benefits, buttonText, buttonLink, badge }) {
    return (
        <div className="bg-card rounded-3xl p-8 border border-border hover:border-primary hover:shadow-2xl hover:shadow-primary/10 hover:scale-[1.02] hover:-translate-y-1 shadow-lg transition-all duration-300 h-full flex flex-col items-start relative overflow-hidden group cursor-pointer">

            <div className='w-full flex justify-between items-start mb-6'>
                <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white group-hover:scale-110 transition-all duration-300">
                    <Icon className="w-7 h-7" strokeWidth={2.5} />
                </div>

                {badge && (
                    <span className="bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wide transition-all duration-300">
                        {badge}
                    </span>
                )}
            </div>
